        ...
        <div className="flex flex-col lg:flex-row items-center justify-between min-h-screen gap-16">
          ...
          <div className={`lg:w-3/5 text-white space-y-8 transition-all duration-1000 ${isVisible ? 'animate-slide-in-left opacity-100' : 'opacity-0'}`}>
            ...
            animationDelay: '0.9s'
              ...
              <Button size="lg" variant="outline" onClick={() => scrollToSection('contact')} className="border-white/30 backdrop-blur-sm transition-all duration-500 px-8 py-3 rounded-full group bg-inherit font-semibold text-white">
                <Play className="w-5 h-5 mr-2 group-hover:animate-bounce" />
                Watch Intro
              </Button>
              <Button size="lg" variant="outline" onClick={() => window.open('https://drive.google.com/file/d/1_TxkdCdpriFzzFEtcHdAFoHmiwibgkoe/view?usp=sharing', '_blank')} className="border-white/30 hover:bg-white/10 backdrop-blur-sm transition-all duration-500 px-8 py-3 rounded-full group font-semibold text-white">
